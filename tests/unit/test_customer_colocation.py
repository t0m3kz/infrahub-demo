...
FirewallContext provisioning mirrors customer_dc.py's, but
customer.parent.firewall_devices/loadbalancer_devices always arrive empty
for Colocation (ColocationMetro doesn't host devices directly, only its
child ColocationZone does) — these tests confirm that no-op and focus on the
hub-and-spoke exchange path, which is unique to Colocation/Cloud/Office.
"""
...
from generators.topology.customer_colocation import DEFAULT_NAMESPACE, CustomerDeploymentColocationExchangeGenerator
...
class TestColocationNoCircuit:
    """_ensure_firewall_context() always runs first now, reading firewall
    devices from customer.parent.devices (no client.filters() round-trip) —
    empty for Colocation since ColocationMetro doesn't host devices directly.
    These assertions only cover the circuit/exchange path."""
...
class TestColocationHubExchange:
    ...
    async def test_wrong_interface_count_skips_exchange(self) -> None:
        ...
        # _ensure_firewall_context is a no-op for Colocation (no client.filters call) —
        # only the circuit interface-count check runs.
        gen.client.filters.assert_not_called()
        gen.client.create.assert_not_called()
        gen.logger.error.assert_called_once()
