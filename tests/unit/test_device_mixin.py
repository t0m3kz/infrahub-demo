...
@pytest.mark.asyncio
async def test_create_devices_accepts_hydrated_flat_interface_shape() -> None:
    """create_devices() should accept flat interfaces from hydrated Template.model_dump()."""
    ...
    template = {
        "id": "tmpl-flat-1",
        "device_type": {"id": "dt-1"},
        "platform": {"name": "checkpoint_gaia"},
        "owner": {"id": "owner-1"},
        "interfaces": [
            {"name": "sync", "role": "ha", "interface_type": "10gbase-x-sfpp"},
            {"name": "eth1", "role": "uplink", "interface_type": "100gbase-x-qsfp28"},
        ],
    }
    ...
    assert names == ["fw-dc101"]
    create_kwargs = gen.client.create.call_args.kwargs
    assert create_kwargs["data"]["interfaces"] == [
        {
            "kind": "DcimPhysicalInterface",
            "data": {"name": "sync", "role": "ha", "interface_type": "10gbase-x-sfpp"},
        },
        {
            "kind": "DcimPhysicalInterface",
            "data": {"name": "eth1", "role": "uplink", "interface_type": "100gbase-x-qsfp28"},
        },
    ]
