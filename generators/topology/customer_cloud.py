"""Customer boarding generator for TopologyCustomerCloud.

Triggered on TopologyCustomerCloud creation (see data/events/99_actions.yml's
trigger-exchange-gateway-on-*-created rules).

Cloud shares the same flat "default" IP namespace as every other customer
...
Auto-Provisioned" section). TopologyCustomerCloud has no ManagedFirewallHA
at all (cloud-native security groups instead — see CloudSecurityGroup), so
this generator's only job is the hub-and-spoke exchange: when this
footprint's circuit terminates on the operator's own hub (e.g. an SD-WAN PoP
with `namespace: INTERNET` set — see
...
Registered as add_customer_deployment_cloud in .infrahub.yml, targeting the
customer_deployments group and querying customer_cloud.gql.
"""
...
class CustomerDeploymentCloudExchangeGenerator(CommonGenerator):
    """add_customer_deployment_cloud — hub-and-spoke exchange (if any) for
    TopologyCustomerCloud. No FirewallContext (Cloud has no ManagedFirewallHA).
    """

    async def generate(self, data: dict[str, Any]) -> None:
        cleaned = clean_data(data)

        entries = cleaned.get("TopologyCustomerCloud", [])
        if not entries:
            self.logger.info("No TopologyCustomerCloud data in GraphQL response — not this generator's kind")
            return
        customer = entries[0]
        ...
        self.logger.info(f"Processing Cloud deployment {customer.get('name', customer_id)}")
    ...
    # operator's hub SD-WAN PoP) — a real VRF boundary. Cloud inherits
