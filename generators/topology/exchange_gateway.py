"""Generator for automatic customer <-> shared-services exchange on customer deployment creation.

Triggered on TopologyCustomerDC/Colocation/Cloud creation (see
data/events/99_actions.yml's trigger-exchange-gateway-on-*-created rules). In
every case it first gets or creates the customer's VRF namespace
({org_id}-{environment}, e.g. "C005-P"), added to the vrf_namespaces group so
add_vrf_namespace (generators/topology/vrf.py) allocates its L3 VNI from the
global pool. What happens next depends on the deployment type — the three
kinds don't reach SHARED-SERVICES the same way:
  ...
  TopologyCustomerCloud       -> requires a TopologyVirtualCircuit between
                                 this cloud region and the DC hosting
                                 SHARED-SERVICES (read via
                                 parent.circuits — CloudRegion inherits
...
Idempotent: re-running for the same customer finds the existing namespace/
...
class ExchangeGatewayGenerator(CommonGenerator):
    ...
    async def generate(self, data: dict[str, Any]) -> None:
        ...
        if customer is None or deployment_kind is None:
            self.logger.error("No TopologyCustomerDC/Colocation/Cloud data in GraphQL response")
            return
        ...
        if not customer_id or not org_id or not environment:
            self.logger.error(
                f"Customer deployment missing id/owner.org_id/environment — cannot proceed "
        ...
        self.logger.info(
            f"Processing {label} customer deployment {customer.get('name', customer_id)} -> namespace {namespace_name}"  # noqa: E501
        )
        ...
        if deployment_kind == "TopologyCustomerDC":
            await self._exchange_via_route_leak(customer_namespace)
        elif deployment_kind == "TopologyCustomerCloud":
            await self._exchange_via_circuit(customer_namespace, customer, namespace_name)
        else:
            self.logger.warning(
    ...
    async def _exchange_via_route_leak(self, customer_namespace: Any) -> None:
    ...
    # ------------------------------------------------------------------
    # Cloud: no fabric EVPN reach -> needs a transport circuit, RoutedExchange
    # on the circuit's own interfaces (the transport hop doubles as the
    # inter-VRF hop).
    # ------------------------------------------------------------------

    async def _exchange_via_circuit(
        self, customer_namespace: Any, customer: dict[str, Any], namespace_name: str
    ) -> None:  # noqa: E501
        ...
        if not virtual_circuits:
            self.logger.warning(
                ...
                "this cloud region yet — skipping shared-services exchange. Wire it manually (or "
