...
def _vpc_blocks_aws(
    ...
    for sg in sgs_by_vpc.get(vpc_name) or []:
        ...
        # Emit inline security group rules
        for rule in sg.get("rules") or []:
            if rule.get("disabled"):
                continue
            rule_name = rule.get("name", "")
            rule_id = _tf_id(f"{sg_name}_{rule_name}")
            direction = rule.get("direction", "ingress")
            # AWS uses separate resource types for ingress vs egress
            res_type = (
            ...
            aws_proto = "-1" if proto == "any" else proto
            attrs = [
                f"security_group_id = aws_security_group.{sg_id}.id",
                f'ip_protocol       = "{aws_proto}"',
            ]
            if proto != "any":
                p_start = rule.get("port_start")
                p_end = rule.get("port_end") or p_start
                if p_start:
                    attrs += [
                        f"from_port = {p_start}",
                        f"to_port   = {p_end}",
                    ]
            if direction == "ingress" and rule.get("source_cidr"):
                attrs.append(f'cidr_ipv4 = "{rule["source_cidr"]}"')
            elif direction == "egress" and rule.get("dest_cidr"):
                attrs.append(f'cidr_ipv4 = "{rule["dest_cidr"]}"')
            else:
...
def _vpc_blocks_azure(
    ...
    for sg in sgs_by_vpc.get(vpc_name) or []:
        ...
        for i, rule in enumerate(sg.get("rules") or []):
            if rule.get("disabled"):
                continue
            rule_name = rule.get("name", "")
            rule_res_id = _tf_id(f"{sg_name}_{rule_name}")
            direction = rule.get("direction", "ingress")
            az_dir = "Inbound" if direction == "ingress" else "Outbound"
            proto = rule.get("protocol", "tcp")
            az_proto = "*" if proto == "any" else proto.capitalize()
            p_start = rule.get("port_start")
            p_end = rule.get("port_end") or p_start
            port_range = (
                f"{p_start}-{p_end}" if p_start and p_end and p_end != p_start else (str(p_start) if p_start else "*")
            )
            src_prefix = rule.get("source_cidr", "*") if direction == "ingress" else "*"
            dst_prefix = rule.get("dest_cidr", "*") if direction == "egress" else "*"
            priority = rule.get("priority") or (100 + i * 10)
            lines += [
                f'resource "azurerm_network_security_rule" "{rule_res_id}" {{',
                f'  name                        = "{rule_name}"',
                f"  resource_group_name         = {rg_ref}",
                f"  network_security_group_name = azurerm_network_security_group.{sg_id}.name",
                f"  priority                    = {priority}",
                f'  direction                   = "{az_dir}"',
                '  access                      = "Allow"',
                f'  protocol                    = "{az_proto}"',
                '  source_port_range           = "*"',
                f'  destination_port_range      = "{port_range}"',
                f'  source_address_prefix       = "{src_prefix}"',
                f'  destination_address_prefix  = "{dst_prefix}"',
                "}",
                "",
            ]
...
def _vpc_blocks_gcp(
    ...
    for sg in sgs_by_vpc.get(vpc_name) or []:
        ...
        active_rules = [r for r in rules_for_sg if not r.get("disabled")]

        if not active_rules:
            # Emit a stub firewall with default allow-tcp
            lines += [
        ...
        # GCP: one google_compute_firewall per direction (ingress/egress rules grouped)
        ingress_rules = [r for r in active_rules if r.get("direction", "ingress") == "ingress"]
        egress_rules = [r for r in active_rules if r.get("direction", "ingress") == "egress"]

        for direction, dir_rules in [("ingress", ingress_rules), ("egress", egress_rules)]:
            if not dir_rules:
                continue
            fw_id = f"{sg_id}_{direction}"
            fw_name = f"{sg_name}-{direction}"
            lines += [f'resource "google_compute_firewall" "{fw_id}" {{']
            lines += [
                f'  name      = "{fw_name}"',
                f"  network   = google_compute_network.{vpc_id}.name",
            ...
            for rule in dir_rules:
                proto = rule.get("protocol", "tcp")
                gcp_proto = "all" if proto == "any" else proto
                p_start = rule.get("port_start")
                p_end = rule.get("port_end")
                if p_start and proto not in ("icmp", "any"):
                    port_str = f"{p_start}-{p_end}" if p_end and p_end != p_start else str(p_start)
                    lines += [
                        "  allow {",
                        f'    protocol = "{gcp_proto}"',
                        f'    ports    = ["{port_str}"]',
                        "  }",
                    ]
                else:
