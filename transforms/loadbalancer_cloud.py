                        ...
                        targets.append(
                            {
                                ...
                                "ip": server["primary_ip"].get("address", "").split("/")[0],
                            }
                        )
                        ...
                        backend_addresses.append(
                            {
                                ...
                                "ip_address": server["ip_address"].get("address", "").split("/")[0],
                            }
                        )
    ...
    for vip in lb.get("vip_services", []):
        ...
        if backend_pool:
            ...
            if not any(s["name"] == pool_name for s in backend_services):
                ...
                for server in backend_pool.get("onprem_servers", []):
                    if server.get("ip_address"):
                        targets.append(
                            {
                                "name": server.get("hostname", ""),
                                "ip": server["ip_address"].get("address", "").split("/")[0],
                            }
                        )
                ...
                # Get health check from VIP service (not backend pool)
                vip_health_checks = vip.get("health_checks", [])
                if vip_health_checks:
                    health_check = vip_health_checks[0]  # Use first health check
                    hc_data = {
