...
uv run infrahubctl generator add_dc name=DC5 --branch you_branch
...
Use `uv run invoke test-integration` for the full multi-vendor matrix. For automatic DC/POD/rack ordering changes, start with `uv run invoke test-integration-routing`; Eurovision for switches deserves at least one rehearsal.
