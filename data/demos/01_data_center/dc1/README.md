...
Pure BGP all the way down. No IGP, no route reflectors, no apologies. Every hop is a BGP session.
Every engineer who joins says "surely there's a simpler way." There isn't.
IPv6 underlay because /127s are cleaner than /31s and someone wanted to feel progressive.
...
### Fabric Scale (Or: How We Learned to Stop Worrying and Love 2-Spine Everything)
...
- **Total Spines:** 6 (2+2+2 - because uniformity in spine count is the ONE thing we got right)
- **Total Racks:** 24 (Because stopping at 20 would suggest we had a plan)

**Deployment Types:** It's complicated—Pod 1: middle_rack (for fans of bureaucracy), Pod 2: mixed (for the indecisive), Pod 3: ToR (for rebels who read "Keep It Simple" and actually believed it). But at least they all have 2 spines now. Small victories.

### Pod Structure (The Family Dysfunction Table)

| Pod   | Spines | Design   | Deployment  | Site Layout | Personality                                      |
| ----- | ------ | -------- | ----------- | ----------- | ------------------------------------------------ |
| POD-1 | 2      | M_MIDDLE | middle_rack | large-dc    | The overachiever with hierarchy complex          |
| POD-2 | 2      | M_MIXED  | mixed       | large-dc    | The confused middle child trying both strategies |
| POD-3 | 2      | S_TOR    | tor         | medium-dc   | The minimalist — flat is a lifestyle choice      |
...
- **Pod Designs:** All 2-spine variants (surprise! 4-spine was just a dream)
  - `S_TOR` - Direct ToR to spine, because flat is life
  - `M_MIXED` - Some ToR, some middle rack, 2 spines holding it together
  - `M_MIDDLE` - Full hierarchy, 2 spines managing the chaos
- **Spine Count:** 2 per pod (non-negotiable, according to bootstrap reality)
- **Super Spine Template:** `N9K-C9336C-FX2_SUPER_SPINE` (not the imaginary edition)
- **Fabric Sorting:** `top_down` (because someone had to make a decision)

**Breaking News:** Turns out those 4-spine designs were just wishful thinking. Bootstrap data doesn't lie, even when our YAML files do. We've learned an important lesson: always check `data/bootstrap/11_pod_designs.yml` before inventing your own reality.
